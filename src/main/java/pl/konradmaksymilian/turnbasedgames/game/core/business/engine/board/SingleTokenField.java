...
public class SingleTokenField<T extends SimplePlayerToken> extends StandardBoardField {
	...
	public void addToken(T token) {
		if (containsToken()) {
			throw new GameEngineException("Cannot add the token to a field; another token is already present on this field");
		} else {
			this.token = token;
		}
	}
	...
	public T removeToken() {
		if (containsToken()) {
			...
			throw new GameEngineException("Cannot remove the token from a field because it is not present on this field");
		}
	}
}
