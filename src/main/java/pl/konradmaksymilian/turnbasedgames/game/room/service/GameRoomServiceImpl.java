...
import pl.konradmaksymilian.turnbasedgames.game.room.dto.message.PublishableHostChangeMessage;
...
import pl.konradmaksymilian.turnbasedgames.game.room.dto.message.PublishableUserJoinMessage;
import pl.konradmaksymilian.turnbasedgames.game.room.dto.message.PublishableUserKickMessage;
...
import pl.konradmaksymilian.turnbasedgames.game.room.dto.message.UserKickMessage;
...
public class GameRoomServiceImpl implements GameRoomService {
	...
	public void deliverCommand(int roomId, GameCommand command) {
		getOrThrowException(roomId).getGameEngine().processCommand(command);
	}
	...
	private void addAllUsersToGame(GameRoom room) {
		...
		if (!engine.containsPlayer(room.getHost().getId())) {
			if (engine.getMaxPlayers() == engine.countPlayers()) {
				engine.removePlayer(engine.getPlayersIds().iterator().next());
			}
